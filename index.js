...
btn.addEventListener("click", function() {
  ...
  if (input == number) {
    ...
    attempts++;
    output.innerHTML = "NOPE!! Its a wrong number!";
    ...
    if (Attempts == 0) {
      ...
      NoAttemptsLeftId.innerHTML = `Oops! Game Over, the correct number was ${number}`;
    }
  }
});
...
button.addEventListener("click", function() {
    let input = document.getElementById("userInput").value;
  if (number > input) {
    output.innerHTML = "Please guess something higher";
  }
   else if (number < input) {
    output.innerHTML = "Please guess something lower";
  }
});
